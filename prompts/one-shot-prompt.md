...
17. **⭐ BILINGUAL (JP/EN) + LANGUAGE TOGGLE — MANDATORY** — Every output HTML must be bilingual (Japanese + English) by default, with a **language toggle button positioned in the top-right corner** of the page chrome. This is non-negotiable regardless of whether the user explicitly requested it. Implementation:
    ...
    - **Units, suffixes, currency symbols and counters are PART of the string and MUST live INSIDE the lang pair — never bare outside it.** The most common toggle bug is leaving a Japanese unit (`万`, `億`, `円`, `名`, `個`, `基`, `件`, `年`, `分`, `兆`, `%`, `pt` …) sitting next to a number that is outside any `lang` span, so it stays Japanese in English mode (e.g. `<span class="lang-ja">100</span><span class="lang-en">100</span>名` ← the `名` leaks). Correct it by wrapping the unit too: either wrap the whole value (`<span class="lang-ja">100名</span><span class="lang-en">100 staff</span>`) or wrap the unit separately (`100<span class="lang-ja">名</span><span class="lang-en"> staff</span>`). When the EN value itself differs (e.g. `272億円` → `¥27.2B`, count-up included), wrap the entire number+unit as a JA/EN pair — do not share the digits.
    - Grep self-check before delivery: every visible text node has both a `ja` and `en` representation, and `<button id="lang-toggle">` (or equivalent) exists in the markup. **Also run a JA-leak check:** parse the DOM (excluding `<script>`/`<style>`) and assert that NO node containing a kana/kanji character (regex `[぀-ヿ一-鿿]`) sits outside both a `ja` and `en` representation. Any hit (a bare unit, a stray label, an avatar initial, etc.) means it will stay Japanese in English mode — wrap it or romanize it before shipping. Target: zero leaks. **If you cannot verify the company identity from the source (e.g., slides are mojibake), DO NOT write claims about the business. Use neutral placeholder copy and disclose the limitation to the user explicitly.**
